...
const startSTTServer = () => {
  ...
  if (fs.existsSync(sttBatchPath)) {
    ...
    sttServerProcess.stdout.on('data', (data: any) => {
      console.log(`STT Server: ${data}`);
    });
    
    sttServerProcess.stderr.on('data', (data: any) => {
      console.error(`STT Server Error: ${data}`);
    });
    
    sttServerProcess.on('close', (code: number) => {
...
function setupIpcHandlers() {
  ...
  ipcMain.handle('save-file', async (_: any, options: SaveFileOptions) => {
  ...
  ipcMain.handle('open-file', async (_: any, options: OpenFileOptions) => {
    if (!mainWindow) return { canceled: true };
